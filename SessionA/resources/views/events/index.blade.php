        ...
        <nav class="col-md-2 d-none d-md-block bg-light sidebar">
            <div class="sidebar-sticky">
                <ul class="nav flex-column">
                    <li class="nav-item"><a class="nav-link active" href="{{route('events')}}">Quản lý sự kiện</a></li>
                </ul>
            </div>
        </nav>
            ...
            <div class="row events">
                ...
                <div class="col-md-4">
                    <div class="card mb-4 shadow-sm">
                        <a href="{{route('event.detail', ['id' => $test->id])}}" class="btn text-left event">
                            <div class="card-body">
                                <h5 class="card-title">{{$test ->name }}</h5>
                                <p class="card-subtitle">{{$test ->date }}</p>
                                <hr>
                                <p class="card-text">{{$registrations->whereIn('ticket_id',$eventTickets->whereIn('event_id', $test->id)->pluck('id'))->count()}}người đăng ký</p>
                            </div>
                        </a>
                    </div>
                </div>
